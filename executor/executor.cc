...
//__declspec(align(2<<11)) // MAX align val : 8192
...
void write_call_output(thread_t* th, bool finished)
{
	...
	debug("reply.call_index ; %lu\n", reply.call_index);
	reply.call_num = th->call_num;
	debug("reply.call_num ; %lu\n", reply.call_num);
	reply.reserrno = reserrno;
	debug("reply.reserrno ; %lu\n", reply.reserrno);
	reply.flags = call_flags;
	debug("reply.flags ; %lu\n", reply.flags);
	reply.signal_size = 0;
	debug("reply.signal_size ; %lu\n", reply.signal_size);
	reply.cover_size = 0;
	debug("reply.cover_size ; %lu\n", reply.cover_size);
	reply.comps_size = 0;
	debug("reply.comps_size ; %lu\n", reply.comps_size);
	if (write(kOutPipeFd, &reply, sizeof(reply)) != sizeof(reply))
