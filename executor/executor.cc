...
// So we define and use uint64_t. Note: pkg/csource does s/uint64_t/uint64_t/.
// Also define uint32_t/16/8 for consistency.
...
const int kInPipeFd = 4;// kMaxFd - 1; // remapped from stdin
const int kOutPipeFd = 5; // kMaxFd - 2; // remapped from stdout
const int kCoverFd = 6;// kOutPipeFd - kMaxThreads;
...
const uint64_t binary_format_native = 0;
const uint64_t binary_format_bigendian = 1;
const uint64_t binary_format_strdec = 2;
const uint64_t binary_format_strhex = 3;
const uint64_t binary_format_stroct = 4;
...
const uint32_t call_flag_finished = 1 << 1;
const uint32_t call_flag_blocked = 1 << 2;
const uint32_t call_flag_fault_injected = 1 << 3;

struct call_reply {
	execute_reply header;
	uint32_t call_indexH;
	uint32_t call_indexL;
	uint32_t call_index;
	uint32_t call_num;
	uint32_t reserrno;
	uint32_t flags;
...
static thread_t* schedule_call(int call_index, int call_num, bool colliding, uint64_t copyout_index, uint64_t num_args, uint64_t* args, uint64_t* pos);
...
static uint64_t read_const_arg(uint64_t** input_posp, uint64_t* size_p, uint64_t* bf, uint64_t* bf_off_p, uint64_t* bf_len_p);
static uint64_t read_result(uint64_t** input_posp);
static uint64_t swap(uint64_t v, uint64_t size, uint64_t bf);
static void copyin(char* addr, uint64_t val, uint64_t size, uint64_t bf, uint64_t bf_off, uint64_t bf_len);
static bool copyout(char* addr, uint64_t size, uint64_t* res);
...
void receive_execute()
{
	...
	for (;;)
	{
		...
		if(rv < 0)
			fail("req_data : control pipe read failed");
		req_pos += rv;
	...
	if (req.prog_size == 0)
		fail("need_prog: no program");
	uint64_t pos = 0;
//	debug("----------------- pos = %lu, req.prog_size = %lu \n", pos, req.prog_size);
	for (;;) {
		
		signed int rv = _read(kInPipeFd, input_data+pos, sizeof(input_data) - pos);
		if (rv < 0)
			fail("prog_data : read failed");
		pos += rv;
...
void reply_execute(int status)
{
	...
	reply.status = status;
	if (write(kOutPipeFd, &reply, sizeof(reply)) != sizeof(reply))
		fail("reply_execute: control pipe write failed");
}
...
retry:
	...
	for (;;) {
		...
		if (call_num == instr_copyin) {
			...
			switch (typ) {
			case arg_const: {
				uint64_t size, bf, bf_off, bf_len;
				uint64_t arg = read_const_arg(&input_pos, &size, &bf, &bf_off, &bf_len);
				copyin(addr, arg, size, bf, bf_off, bf_len);
				break;
			}
			case arg_result: {
				uint64_t meta = read_input(&input_pos);
				uint64_t size = meta & 0xff;
				uint64_t bf = meta >> 8;
			...
			case arg_csum: {
				...
				case arg_csum_inet: {
					...
					uint16_t csum_value = csum_inet_digest(&csum);
		...
		if (colliding && (call_index % 2) == 0) {
			...
			const uint64_t timeout_ms = flag_debug ? 1000 : 45;
	...
	if (!colliding && !collide && running > 0) {
		...
		uint64_t wait_end = wait_start + wait;
...
thread_t* schedule_call(int call_index, int call_num, bool colliding, uint64_t copyout_index, uint64_t num_args, uint64_t* args, uint64_t* pos)
{
...
void write_coverage_signal(cover_t* cov, uint32_t* signal_count_pos, uint32_t* cover_count_pos)
{
...
{
	uint32_t reserrno = 999;
	uint32_t call_flags = call_flag_executed;
	...
	if (flag_collect_comps) {
		...
		uint32_t ncomps = th->cov.size;
		kcov_comparison_t* start = (kcov_comparison_t*)(th->cov.data + sizeof(uint64_t));
		...
		*comps_count_pos = comps_size;
		...
		if (is_kernel_64_bit)
			write_coverage_signal<uint64_t>(&th->cov, signal_count_pos, cover_count_pos);
		else
			write_coverage_signal<uint32_t>(&th->cov, signal_count_pos, cover_count_pos);
	}
	...
	reply.call_indexH = (th->call_index)>>3 & 0b111;
	reply.call_indexL = (th->call_index) & 0b111;
	reply.call_index  = 0;
//	reply.buf =  ((th->call_num) >> 10) & 0x0f;
//	reply.call_numH =  ((th->call_num) >> 8) & 0b11;
//	debug("reply.call_numH : %lu\n", reply.call_numH);
//	reply.call_numL = th->call_num & 0xff;
	reply.call_num = th->call_num;
//	debug("reply.call_numL : %lu\n", reply.call_numL)	;
	...
	reply.comps_size = 0;
	if (write(kOutPipeFd, &reply, sizeof(reply)) != sizeof(reply))
		fail("write_call_output: control pipe call write failed");
...
void write_extra_output()
{
	...
	if (is_kernel_64_bit)
		write_coverage_signal<uint64_t>(&extra_cov, signal_count_pos, cover_count_pos);
	else
		write_coverage_signal<uint32_t>(&extra_cov, signal_count_pos, cover_count_pos);
	cover_reset(&extra_cov);
...
template <typename T>
void copyin_int(char* addr, uint64_t val, uint64_t bf, uint64_t bf_off, uint64_t bf_len)
{
...
void copyin(char* addr, uint64_t val, uint64_t size, uint64_t bf, uint64_t bf_off, uint64_t bf_len)
{
	...
	case binary_format_bigendian:
		NONFAILING(switch (size) {
			case 1:
				copyin_int<uint8_t>(addr, val, bf, bf_off, bf_len);
				break;
			case 2:
				copyin_int<uint16_t>(addr, val, bf, bf_off, bf_len);
				break;
			case 4:
				copyin_int<uint32_t>(addr, val, bf, bf_off, bf_len);
				break;
			case 8:
				copyin_int<uint64_t>(addr, val, bf, bf_off, bf_len);
				break;
			default:
...
bool copyout(char* addr, uint64_t size, uint64_t* res)
{
//	bool ok = false;
	...
	//_ATOMIC_STORE(&ok, true, std::memory_order_release)
	return ok;
}
...
uint64_t read_arg(uint64_t** input_posp)
{
	...
	case arg_const: {
		uint64_t size, bf, bf_off, bf_len;
		uint64_t val = read_const_arg(input_posp, &size, &bf, &bf_off, &bf_len);
...
uint64_t swap(uint64_t v, uint64_t size, uint64_t bf)
{
	...
	switch (size) {
	case 2:
		debug("_byteswap_ushort\n");
		return _byteswap_ushort(v);
	case 4:
		debug("_byteswap_ulong\n");
		return _byteswap_ulong(v);
	case 8:
		debug("_byteswap_uint64\n");
...
uint64_t read_const_arg(uint64_t** input_posp, uint64_t* size_p, uint64_t* bf_p, uint64_t* bf_off_p, uint64_t* bf_len_p)
{
	uint64_t meta = read_input(input_posp);
	uint64_t val = read_input(input_posp);
	*size_p = meta & 0xff;
	uint64_t bf = (meta >> 8) & 0xff;
...
void kcov_comparison_t::write()
{
	...
	case KCOV_CMP_SIZE1:
		arg1 = (uint64_t)(long long)(signed char)arg1;
		arg2 = (uint64_t)(long long)(signed char)arg2;
		break;
	case KCOV_CMP_SIZE2:
		arg1 = (uint64_t)(long long)(short)arg1;
		arg2 = (uint64_t)(long long)(short)arg2;
		break;
	case KCOV_CMP_SIZE4:
		arg1 = (uint64_t)(long long)(int)arg1;
		arg2 = (uint64_t)(long long)(int)arg2;
		break;
	}
...
bool kcov_comparison_t::ignore() const
{
	...
	if ((type & KCOV_CMP_SIZE_MASK) == KCOV_CMP_SIZE8) {
		...
		uint64_t out_start = (uint64_t)output_data;
